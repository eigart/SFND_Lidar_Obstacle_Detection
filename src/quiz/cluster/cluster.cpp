...
void clusterHelper(int& idx, const std::vector<std::vector<float>> points, std::vector<int>& cluster, std::vector<bool>& marked, KdTree* tree, float distanceTol)
{
	marked[idx] = true;
	...
	for(int id : nearest)
	{
		if(!marked[id])
		{
			clusterHelper(id, points, cluster, marked, tree, distanceTol);
		}
	}
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	while (i < points.size())
	{
		...
		clusterHelper(i, points, cluster, marked, tree, distanceTol);
